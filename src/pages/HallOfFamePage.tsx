...
const HallOfFamePage: React.FC = () => {
  ...
  const completedMissionIdsForSelectedDate = useMemo(() => {
    return new Set(
      missionLogsForSelectedDate.map(
        (log) => log.mission_id
      )
    );
  ...
  const displayedMissionsForSelectedDate = useMemo(() => {
    ...
    return missionsFromSnapshot
      .map((mission: Mission) => ({
        ...mission,
        isCompleted: completedMissionIdsForSelectedDate.has(
          mission.id
        ),
      }))
  ...
  // 보상 표시 관련 상태 추가
  const [selectedBadgeReward, setSelectedBadgeReward] =
  ...
  const [updatingReward, setUpdatingReward] =
    useState(false);

  // 배지를 클릭했을 때 주간 보상 표시 토글 함수
  const toggleRewardDisplay = (
  ...
  const handleOpenBadgeSelectionModal = (week: string) => {
    ...
    if (!pendingBadge) {
      console.log(
        `[ERROR] 해당 주(${week})의 미선택 배지를 찾을 수 없습니다.`
      );
      toast.error(
        "해당 주의 미선택 배지를 찾을 수 없습니다."
      );
      return;
    }
  ...
  const handleBadgeSelect = async (badgeId: string) => {
    ...
    try {
      console.log(
        `[DEBUG] 배지 선택: ${badgeId}, 주: ${selectedWeek}`
      );
      ...
      if (!pendingBadge) {
        ...
        toast.error(
          "해당 주차의 미션 정보를 찾을 수 없습니다."
        );
        return;
      }

      console.log(
        `[DEBUG] 선택한 배지: ${badgeId}, 미션 정보:`,
        pendingBadge
      );

      // 1. badges 테이블에 해당 배지가 있는지 확인
      const { data: existingBadge, error: checkError } =
        await supabase
          .from("badges")
          .select("id")
          .eq("id", badgeId)
      ...
      if (!existingBadge) {
        ...
        let badgeDescription =
          "주간 미션을 모두 완료하여 획득한 배지입니다.";

        // weekly_streak_1 배지 정보 가져와서 사용
        try {
          const {
            data: weeklyStreakBadge,
            error: weeklyError,
          } = await supabase
            .from("badges")
        ...
        console.log(
          `[DEBUG] 배지 ID ${badgeId}를 badges 테이블에 추가 완료`
        );
      }
  ...
  const loadPendingBadges = useCallback(async () => {
    ...
    try {
      console.log(
        "[DEBUG] 미선택 배지 확인 시작 - loadPendingBadges()"
      );
      console.log(
        "[DEBUG] 현재 로그인한 사용자 ID:",
        user.id
      );

      // 1. 사용자의 모든 weekly_streak_1 배지 가져오기
      const {
        data: weeklyStreakBadges,
        error: weeklyError,
      } = await supabase
        .from("earned_badges")
      ...
      if (weeklyError) {
        console.error(
          "[ERROR] weekly_streak_1 배지 조회 오류:",
          weeklyError
        );
        return;
      }
      ...
      console.log(
        "[DEBUG] 조회된 weekly_streak_1 배지:",
        weeklyStreakBadges
      );
      console.log(
        "[DEBUG] 조회 쿼리 조건 - student_id:",
        user.id
      );

      if (
        !weeklyStreakBadges ||
        weeklyStreakBadges.length === 0
      ) {
        console.log(
          "[DEBUG] weekly_streak_1 배지가 없습니다."
        );
      ...
      const { data: allCustomBadges, error: customError } =
        await supabase
          .from("earned_badges")
          .select("id, badge_id, earned_date")
          .eq("student_id", user.id)
      ...
      for (const weeklyBadge of weeklyStreakBadges) {
        ...
        // 주의 시작일과 종료일 계산
        const earnedDateClone = new Date(
          earnedDate.getTime()
        );
        const day = earnedDateClone.getDay(); // 0: 일요일, 1: 월요일, ..., 6: 토요일
        const diff =
          earnedDateClone.getDate() -
          day +
          (day === 0 ? -6 : 1); // 이번 주 월요일 날짜 계산

        const weekStart = new Date(
          earnedDateClone.getTime()
        );
        ...
        const customBadgesInWeek =
          allCustomBadges?.filter((customBadge) => {
            const customDate = new Date(
              customBadge.earned_date + "T00:00:00"
            );
            const isInRange =
        ...
        if (customBadgesInWeek.length === 0) {
          ...
          pendingBadges.push({
            id: weeklyBadge.id,
            earned_at: weeklyBadge.earned_date, // 타입 호환성을 위해 earned_at으로 유지
            reward_text: null, // reward_text 없음
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              <h2
                ...
                <LuCalendarDays className="mr-2" /> 날짜별
                기록 조회
              </h2>
              ...
              {dailySnapshot &&
                displayedMissionsForSelectedDate.length ===
                  0 && (
                  <p className="text-center text-gray-500">
                ...
                {displayedBadges.map((earnedBadge) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        toggleRewardDisplay(earnedBadge)
                      }>
                      {/* 보상 정보가 있고 아직 사용하지 않은 경우에만 알림 배지 표시 */}
                      {hasReward &&
                        !earnedBadge.reward_used && (
                          <div
                            className="absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center text-white shadow-md z-10"
                            style={{
                              backgroundColor:
